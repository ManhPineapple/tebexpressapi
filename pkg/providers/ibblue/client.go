...
func NewClient(domain string, apiKey string) *Client {
	// username and password split from api key (username:password)
	auth := strings.Split(apiKey, ":")
	log.Println(domain)
	log.Println(auth)
	newClient := &Client{Client: &http.Client{}, BaseURL: domain}
	if len(auth) == 2 {
		newClient.UserName = auth[0]
		newClient.Password = auth[1]
	}
	return newClient
}
