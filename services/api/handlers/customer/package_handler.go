...
func (h *PackageHandler) List() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for i, Package := range packages {
			...
			newPackage.ScanWeightAt = Package.ScanWeightAt
...
func (h *PackageHandler) Update() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, UpdatePackageResponse{Success: true, Package: packageDTO, DeliverLogs: deliverLogs, ExtraFree: extraFee})
	}
}
